...
public class DataStore {
	...
	public List<Task> findAll(String packageName) {
		PersistenceManager pm = PMF.get().getPersistenceManager();
		  try {
			  
		      Query query = pm.newQuery("select from " + Task.class.getName()
		          + " where emailAddress=='" + getUserEmail() + "'"+" && appPackageName=='" + packageName + "'");
			  //Query query = pm.newQuery("select from " + Task.class.getName()
			  //          + " where id==" + id.toString() + " && emailAddress=='" + getUserEmail() + "'");
			  List list = (List) query.execute();
		      if (list.size() == 0) {
	...
	public List<Task> findAll(String packageName, Long timestamp) {
		PersistenceManager pm = PMF.get().getPersistenceManager();
		  try {
			  
		      Query query = pm.newQuery("select from " + Task.class.getName()
		          + " where emailAddress=='" + getUserEmail() + "'"+" && appPackageName=='" + packageName + "'"+" && timestamp>" + timestamp + "");
			  //Query query = pm.newQuery("select from " + Task.class.getName()
			  //          + " where id==" + id.toString() + " && emailAddress=='" + getUserEmail() + "'");
			  List list = (List) query.execute();
		      if (list.size() == 0) {
	...
	public List<Task> findExactTimeStamp(String packageName, Long timestamp) {
	...
	public List<Task> findGoogleIdList(String packageName, List<Long> idList) {
		// TODO Auto-generated method stub
		List<Task> list = findAll(packageName);
