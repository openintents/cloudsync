...
public class DetectChange {
	...
	public DetectChange(Context context) {
	...
	void detectExecute() {
		...
		detectPostExecute( new String[]{packJson(),packDelJson()} );
	}
	
	private void detectPostExecute(String[] result) {
		...
		SyncTotal st = new SyncTotal(context);
		st.syncExecute(new String[]{jsonString,jsonDeleteString});
